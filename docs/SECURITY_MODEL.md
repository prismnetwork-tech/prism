...
## Which machine the renter is talking to

A grant names an address. Until the SSH host key on the far end is checked,
anything that can reach that address can accept the session, so every class
publishes what it can about that key and the clients Prism ships check it before
opening one.

An `attested` lease publishes the fingerprint out of the guest report, whose
`REPORT_DATA` commits to the key the guest generated at boot. That holds against
the operator as well as against everyone else on the path. It does not hold
against us: the report is walked to AMD's root in the control plane, and the
client compares the host key to the fingerprint that walk published rather than
to the report itself. Removing us from that set is what the zkVM route below is
for.

A self-hosted `open` or `isolated` lease publishes the fingerprint the node put
on the signed report that opened access. The claim is the operator's, under the
device key their bond stands behind, so it rules out the access relay, the
network path and anyone between the node and the renter, and it turns a
substitution by the operator into a signed statement the dispute queue can act
on. It is not equivalent to the report, and the grant marks the difference:
`channel_key_source` is `snp_report` or `node_report`, never both and never
neither.

Brokered capacity publishes nothing. The instance's host key is generated by the
cloud, the cloud's API does not expose it, and the network never sees it, so
there is no honest pin to offer. The shipped clients record the key on first
sight and hold it for the rest of the lease, which catches a machine swapped in
partway through and cannot catch one that was wrong from the start. A renter who
needs better than that sets `requireHostKey` and takes the refusal, or raises
the trust floor off brokered capacity.
...
- Proof and X publication must remain separate from settlement so an X API failure
  ...
  yet.
- The paid gateways (`@prismnetwork/x402`, `@prismnetwork/inference-gateway`) hold
  a funded wallet and answer routes that spend it, so they listen on `127.0.0.1`
  and refuse to start on a wider address without a credential every request must
  carry. Exposing one publicly means putting a reverse proxy in front and giving
  the proxy that credential.

## Paying without being robbed in transit

A payment on the legacy rail is a transfer that has already settled plus a
signature naming who made it. Signing the transaction hash alone proved who paid
and not what for, so anyone who read the header off an unencrypted hop could send
it back with a command or a prompt of their own and take the answer, leaving the
payer with a consumed payment. The payer now signs the transaction together with
the SHA-256 of the request body, and the endpoint recomputes that digest from the
bytes that arrived. `PRISM_X402_ALLOW_UNBOUND_PAYMENT=1` reopens the old form for
the length of a migration.

This covers the legacy rail. The `exact` scheme, where the payer signs an EIP-3009
authorization the endpoint broadcasts, is still keyed on the payer and the nonce
alone and carries no equivalent binding, because the scheme as published has no
field to put one in.
