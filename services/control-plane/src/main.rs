    ...
    async fn lease_access(
        ...
        match self {
            Self::Memory(market) => {
                ...
                if class_for_lease(lease_id, &lease.node_id, node_class, verdict, now)
                    < quoted_class
                {
            ...
            Self::Postgres(pool) => {
                ...
                if let Some((host, port, expires_at)) = direct {
                    return Ok(Some(LeaseAccessGrant {
                        ...
                        // up.
                        channel_key: channel_key(attested_channel_key, None),
                    }));
                }
                let stored = query_as::<
                    _,
                ...
                Ok(stored.map(
                    |(SqlJson(token), SqlJson(jupyter_token), expires_at, reported)| {
                        LeaseAccessGrant {
                            ...
                            channel_key: channel_key(attested_channel_key, reported),
                        }
                    },
                ))
            }
...
/// cannot lie about it.
fn channel_key(attested: Option<String>, reported: Option<String>) -> Option<ChannelKey> {
    attested
        .map(|fingerprint| ChannelKey {
            fingerprint,
    ...
    Migrator {
        migrations: Cow::Owned(vec![
            ...
            Migration::new(
                ...
                MigrationType::Simple,
                Cow::Borrowed(include_str!("../migrations/0020_lease_channel_key.sql")),
                false,
            ),
        ]),
        ..Migrator::DEFAULT
